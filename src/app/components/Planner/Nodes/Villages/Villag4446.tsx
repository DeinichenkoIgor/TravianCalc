...
const Nodes4446 = ({ position, onDelete, serverSpeed }: { position: number; onDelete: () => void; serverSpeed: number; }) => {
  ...
  useEffect(() => {
    ...
    if (!savedNodes.includes(position)) {
      savedNodes.push(position);
      localStorage.setItem("nodes", JSON.stringify(savedNodes));
    }
  }, [position]);
          ...
          >
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const savedNodes = JSON.parse(localStorage.getItem("nodes") || "[]");
                  localStorage.setItem("nodes", JSON.stringify(savedNodes.filter((id: number) => id !== position)));
                  closeAlert(); // Закрываем окно выбора
                }}
              >
                Удалить
